    ...
    def update(self, success=None, failure=None):
        ...
        if failure:
            ...
            for inst in self.insts_own:
                if inst in self.data and self.data[inst]["status"]:
...
class StatArray:
    ...
    def make_vbs(self, addit_key=None):
        ...
        for inst in self.inst_full:
            alts = []
            for stat_obj in self.stat_objs:
                if inst in stat_obj.data and stat_obj.data[inst]["status"]:
    ...
    def compare(self, cmp_key=None):
        ...
        if cmp_key:
            for inst in self.inst_full:
                ...
                for stat_obj in self.stat_objs:
                    if (
                        inst in stat_obj.data
                        and stat_obj.data[inst]["status"]
                        and cmp_key in stat_obj.data[inst]
                    ):
