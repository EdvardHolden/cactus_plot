...
def load_json(stat_arr, options):
    ...
    if options["replace"]:
        data = [
            (options["replace"][n], v, s, l) if n in options["replace"] else (n, v, s, l)
            for n, v, s, l in data
        ]
...
def load_csv(names, stats, options):
    ...
    if options["replace"]:
        names = [options["replace"][n] if n in options["replace"] else n for n in names]
